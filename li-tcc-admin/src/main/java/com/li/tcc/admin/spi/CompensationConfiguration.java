package com.li.tcc.admin.spi;

import com.google.common.base.Splitter;

import com.li.tcc.common.jedis.JedisClient;
import com.li.tcc.common.jedis.JedisClientCluster;
import com.li.tcc.common.jedis.JedisClientSingle;
import com.li.tcc.common.serializer.ObjectSerializer;

import com.li.tcc.admin.service.CompensationService;
import com.li.tcc.admin.service.compensate.*;
...
public class CompensationConfiguration {
	...
	static class RedisRecoverConfiguration {
		...
		public CompensationService redisTransactionRecoverService() {
			...
			if (cluster) {
				...
				final Set<HostAndPort> hostAndPorts = Splitter.on(clusterUrl).splitToList(";").stream()
				...
				if (StringUtils.isNoneBlank(password)) {
					jedisPool = new JedisPool(config, hostName, Integer.parseInt(port), 30, password);
				} else {
					jedisPool = new JedisPool(config, hostName, Integer.parseInt(port), 30);
				}
	...
	static class MongoRecoverConfiguration {
		...
		public CompensationService mongoTransactionRecoverService() {
			...
			MongoCredential credential = MongoCredential.createScramSha1Credential(env
					.getProperty("compensation.mongo.userName"), env.getProperty("compensation.mongo.dbName"), env
					.getProperty("compensation.mongo.password").toCharArray());
			...
			try {
				...
				mongoTemplate = new MongoTemplate(clientFactoryBean.getObject(),
