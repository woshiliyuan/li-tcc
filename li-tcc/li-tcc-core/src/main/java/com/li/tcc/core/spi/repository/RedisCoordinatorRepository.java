...
import com.li.tcc.common.config.TccRedisConfig;
...
public class RedisCoordinatorRepository implements CoordinatorRepository {
	...
	public int updateParticipant(final TccTransaction tccTransaction) {
		...
		try {
			CoordinatorRepositoryAdapter adapter = objectSerializer.deSerialize(contents,
	...
	private void buildJedisPool(final TccRedisConfig tccRedisConfig) {
		...
		if (tccRedisConfig.getCluster()) {
			...
			if (StringUtils.isNoneBlank(tccRedisConfig.getPassword())) {
				jedisPool = new JedisPool(config, tccRedisConfig.getHostName(), tccRedisConfig.getPort(),
						tccRedisConfig.getTimeOut(), tccRedisConfig.getPassword());
			} else {
				jedisPool = new JedisPool(config, tccRedisConfig.getHostName(), tccRedisConfig.getPort(),
						tccRedisConfig.getTimeOut());
			}
